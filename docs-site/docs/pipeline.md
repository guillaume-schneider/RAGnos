...
2. Discover all supported corpus files (`.pdf`, `.json`) in `DOCS_DIR`.
...
6. If not, load documents, split them, create embeddings, and build a fresh Chroma index.
...
2. Validate that at least one supported corpus file exists.
